...
public class Race {
	...
	private LocalTime localTime = LocalTime.of(10, 00, 00, 00);
	private int speedSimulator = 1; 
	
	public Race() {}

	public Race(Track track, ArrayList<Skier> skiers) {
		this.track = track;
		this.skiers = skiers;
	}
	
	public Race(Track track, ArrayList<Skier> skiers, int milliseconds) {
		this.track = track;
		this.skiers = skiers;
		this.milliseconds = milliseconds;
	}
	
	public Race(Track track, ArrayList<Skier> skiers, int milliseconds, LocalTime localTime) {
		this.track = track;
		this.skiers = skiers;
		this.milliseconds = milliseconds;
	...
	private void skierCrossedLine(Skier skier) {
		if (skier.getDistance() >= track.getDistance()) {
			...
			System.out.println(skier.getFinishTime());
		}
	}
	...
	public void addMiddleTimeToSkier(Skier skier, double distance) {
	    if (skier.getDistance() >= distance) {
            ...
            System.out.println("Middle Time at photocell distance: " + distance + "m : " 
            	+ skier.getName() + " : "
            	+ skier.getTimer().getMiddleTimes().getLast());
	    }
	}
}
